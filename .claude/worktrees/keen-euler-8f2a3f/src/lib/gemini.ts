...
export async function performComprehensiveAnalysis(assignmentData: string, rubricData: string, feedbackData: string) {
  try {
    ...
    throw new Error("The Institutional Analysis Engine encountered a protocol disruption. Please verify all evidence objects are legible.");
  }
}

export async function chatWithAdvocate(message: string, history: { role: 'user' | 'model', text: string }[]) {
  try {
    const chat = ai.chats.create({
      model: "gemini-2.5-flash-preview-05-20",
      config: {
        systemInstruction: "You are an Expert Academic Advocate. Your goal is to help university students analyze their grading rubrics and case evidentiary data to build strong grade appeals. Use professional, analytical, and institutional tone. Be precise about identifying potential discrepancies in rubrics or calculation errors. Always encourage institutional integrity while being a firm advocate for the student's merit.",
      },
    ...
    throw new Error("Failed to communicate with the Institutional Advocacy System. Please check your neural link (API KEY).");
  }
}
