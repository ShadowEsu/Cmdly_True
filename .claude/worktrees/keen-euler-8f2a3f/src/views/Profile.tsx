...
const Profile: React.FC = () => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    const fullContent = `${profileData.name} ${profileData.major} ${profileData.studentId}`;
    ...
    const sanitized = {
      ...
      studentId: sanitizeInput(profileData.studentId),
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-center lg:items-end">
          ...
          <div className="flex-1 text-center lg:text-left space-y-4">
            <div className="flex items-center justify-center lg:justify-start gap-4">
               <span className="text-xs font-black uppercase tracking-[0.6em] text-primary/40">Your Profile</span>
            ...
            <div className="flex items-center justify-center lg:justify-start gap-6 pt-4">
              <div className="flex items-center gap-2 px-6 py-2 bg-green-500/10 rounded-full border border-green-500/20">
                ...
                <span className="text-xs font-black text-green-700 uppercase tracking-widest">Session Verified</span>
              </div>
              <p className="text-xs font-bold text-on-surface-variant/40 uppercase tracking-[0.4em]">
                {user.email}
              </p>
            </div>
          </div>
            ...
            {!isEditing ? (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-12">
                    <div className="group">
                      <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-primary/30 mb-2">Student ID</p>
                      <p className="font-serif text-4xl font-light text-primary italic uppercase tracking-tight">{profileData.studentId || 'Not set yet'}</p>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold uppercase tracking-widest opacity-40 ml-4">Student ID#</label>
                    <input 
                      type="text"
                      required
                      value={profileData.studentId}
                      onChange={(e) => setProfileData({...profileData, studentId: e.target.value})}
                      className="w-full bg-surface/50 border border-primary/10 rounded-[2rem] px-8 py-5 outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary/30 text-xl font-mono uppercase"
                    />
                  </div>
