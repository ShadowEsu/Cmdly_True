...
const Logo: React.FC<LogoProps> = ({ className = '', size = 'lg', showTagline = false }) => {
  ...
  return (
    <div className={`flex flex-col flex-1 items-center justify-center p-8 ${className}`}>
      ...
      >
        <h1 className={`font-serif ${sizes[size]} text-primary font-light tracking-widest mb-3 uppercase`}>
          REGRADE
        </h1>
        <div className={`h-px w-20 bg-primary/20 mx-auto ${showTagline ? 'mb-4' : ''}`} />
      </motion.div>
      {showTagline && (
        <p className={`text-on-surface-variant font-medium opacity-50 ${taglineSizes[size]} uppercase tracking-[0.4em] text-center`}>
